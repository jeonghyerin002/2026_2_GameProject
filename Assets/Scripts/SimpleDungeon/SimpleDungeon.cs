...
public class SimpleDungeon : MonoBehaviour
{
    ...
    private readonly Dictionary<Vector2Int, Room> rooms = new(); //방의 중심 좌표를 key로 사용하여 각 room 정보를 지정합니다.
    ...
    {
        ClearGenerated();
        CreateRoomsAndCorrider(); //기존에 생성되어있던 던전을 삭제
        AssignSpecialRooms(); //방을 생성하고 방 사이를 복도로 연결
    ...
    void AssignSpecialRooms() //생성된 일반 방 중에 보스방과 보물방을 지정합니다.
    {
        Room boss = null;
        int farthest = -1;
        List<Room> normalRooms = new();

        foreach (Room room in rooms.Values) //생성된 모든 방 검사
        {
            if (room.type == RoomType.Start) continue; //시작방은 보스방 제외
        ...
        {
            if (room.type == RoomType.Normal) normalRooms.Add(room); //남아 있는 일반 방만 보물 방 후보로 추가
        }
        ...
        for (int i = 0; i < treasureCount; i++)
        {
            ...
            normalRooms[index].type = RoomType.Treasure; //선택된 일반 방을 보물 방으로 변경
            normalRooms.RemoveAt(index); //후보에서 제거한다. (중복되지 않게)
        }
    }

    private void CreateRoomsAndCorrider()
    {
        int startSize = Random.Range(minRoomSize, maxRoomSize + 1);

        TryAddRoom(Vector2Int.zero, startSize, RoomType.Start); //던전의 원점에 시작 방을 생성한다

        int attenpts = 0; //현재까지 방 배치를 시도한 횟수

        while (rooms.Count < roomCount && attenpts < maxPlacementAttempts) //목표 방 개수에 도달하거나 최대 시도 횟수를 넘을 때까지 반복
        {
            //현재 생성되어있는 방들을 List로 복사
            List<Room> existingRoom = new(rooms.Values);
            Room parent = existingRoom[Random.Range(0, existingRoom.Count)];
            List<Vector2Int> directions = ShuffledDirections();

            foreach(Vector2Int direction in directions)
            {
                Vector2Int center = parent.center + direction * roomSpacing;
        ...
        {
            Debug.LogWarning($"방 {roomCount} 개 중 {rooms.Count} 개를 생성했습니다.");
        }
    }

    static List<Vector2Int> ShuffledDirections()
    {
        List<Vector2Int> result = new(Directions); //원본 방향 배열을 수정하지 않도록 새로운 List 만든다
        for (int i = result.Count - 1; i > 0; i--) //뒤쪽 원소부터 하나씩 랜던 위치와 교환하는 Fisher-Yates 방식
        {
    ...
    {
        //방을 중심으로 방이 시작되는 상대 좌표를 계상
        int min = -size / 2;
        int max = min + size;

        //기존 방 주변 한 칸의 여백까지 검사하여 방끼리 붙지 않게 합니다.
        for (int x = min - 1; x <= max; x++) //X 범위보다 한 칸
        {
            for (int y = min - 1; y <= max; y++) //Y 범위보다 한 칸
            {
        ...
        for (int x = min; x < max; x++)
        {
            ...
            {
                Vector2Int cell = center + new Vector2Int(x, y); //방 중심에 상대 좌표를 더하여 실제 셀 좌표를 계산합니다
    ...
    private void CreateCorridor(Vector2Int start, Vector2Int end) //시작 방과 끝 방 사이에 ㄱ자 형태의 복도를 생성
    {
        ...
        if (Random.value < 0.5f) //50% 확률로 x축과 y축 중 어느 방향을 먼저 이동할지 결정한다.
        {
    ...
    void WalkX(ref Vector2Int current, int target)
    {
        while (current.x != target) //현재 x좌표가 목표 x좌표와 같아질 때까지 반복
        {
            floors.Add(current);
            current.x += current.x < target ? 1 : -1; //목표가 오른쪽임녀 +1, 왼쪽이면 -1씩 X 좌표 이동
        }
    }
    void WalkY(ref Vector2Int current, int target)
    {
        while (current.y != target) //현재 y좌표가 목표 y좌표와 같아질 때까지 반복
        {
            floors.Add(current);
            current.y += current.y < target ? 1 : -1; //목표가 오른쪽임녀 +1, 왼쪽이면 -1씩 X 좌표 이동
        }
    }
    ...
    {
        rooms.Clear(); //저장되어있던 모든 방 정보를 삭제합니다
        floors.Clear(); //저장되어있던 모든 바닥과 복도 좌표를 삭제합니다.
        roomFloors.Clear(); //저장되어있던 방 전용 바닥 좌표를 삭제합니다.

        Transform oldRoot = transform.Find("Generated Dungeon");

        if (oldRoot == null) return; //기존에 생설된 던전이 없다면 종료

        if (Application.isPlaying) Destroy(oldRoot.gameObject);
        else DestroyImmediate(oldRoot.gameObject); //에디터 모드라면 즉시 삭제할 수 있는 DestroyImmediate를 사용합니다.
    }
